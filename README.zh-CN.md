...
纠错和撤销也可以通过语音触发，但触发规则是故意收紧的：

- 语音撤销只有在整段语音基本只包含 undo 关键词时才会触发，例如“撤销”或“undo”。如果同一段里还有其他任务内容，就会按普通 append 文本处理。
- 语音纠错必须先说 correction prefix，例如“纠错”，短暂停顿后再说纠错内容。例如：“纠错……删除第一句”或“纠错……第二句改成更简洁”。如果没有这个前缀，“第二句改成……”这类内容会按 append 处理。
- 这些关键词可以在 `config.yaml` 的 `voice_queue.command_keywords.undo` 和 `voice_queue.command_keywords.correction_prefixes` 中自定义。

按键仍然是显式快捷入口：按住 `c` 可以在不说前缀的情况下强制 correction mode，按 `u` 可以确定性撤销。
...
- `voice_queue`：语言提示和语音触发 undo/correction 的关键词。可以改 `command_keywords.undo` 来自定义撤销关键词，改 `command_keywords.correction_prefixes` 来自定义纠错前缀。
- `output`：剪贴板输出行为。
